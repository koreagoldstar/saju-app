...
async function generateDatasetBasedFortune(input) {
  ...
  if (mode === "llm") {
    const llmKey = buildCacheKey(input, category, tone) + "|llm";
    const ttlMs = getCacheTtlMs();
    const now = Date.now();
    ...
    if (llmResult && llmResult.text) {
      const payload = {
        ...
        category,
        categoryLabel: CATEGORY_LABELS[category],
        keywords: buildMockKeywords(input),
  ...
  const templateResult = await generateTemplateBasedFortune(input, category, tone);
  return {
    ...templateResult,
