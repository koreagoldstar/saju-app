...
/** KST 기준 '오늘'의 양력 일자와 그날의 일간지(日干支). 일일 운세 변별에 사용합니다. */
function getFortuneDailyContext() {
  ...
  const todayDayPillar = eightChar.getDayGan() + eightChar.getDayZhi();
  const fortuneDateKey =
...
/** KST 기준 '내일' 양력 일자와 그날의 일간지(日干支). 내일 운세 변별에 사용합니다. */
function getFortuneTomorrowContext() {
  const formatter = new Intl.DateTimeFormat("en-CA", {
    timeZone: "Asia/Seoul",
    year: "numeric",
    month: "2-digit",
  ...
  const solarTomorrow = solarToday.nextDay(1);
  const y2 = solarTomorrow.getYear();
  const m2 = solarTomorrow.getMonth();
  const d2 = solarTomorrow.getDay();
  const lunar = solarTomorrow.getLunar();
  const eightChar = lunar.getEightChar();
  if (typeof eightChar.setSect === "function") {
    eightChar.setSect(2);
  }
  const tomorrowGan = eightChar.getDayGan();
  const tomorrowZhi = eightChar.getDayZhi();
  const tomorrowDayPillar = tomorrowGan + tomorrowZhi;
  const fortuneTomorrowKey =
    String(y2) + "-" + String(m2).padStart(2, "0") + "-" + String(d2).padStart(2, "0");
  return {
    fortuneTomorrowKey,
    tomorrowDayPillar,
    tomorrowGan,
...
module.exports = {
  getSajuFromInput,
  getFortuneDailyContext,
  getFortuneTomorrowContext,
  getElementFromGan,
};